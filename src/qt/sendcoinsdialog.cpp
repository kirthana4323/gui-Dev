...
void SendCoinsDialog::setModel(WalletModel *_model)
{
    ...
    if(_model && _model->getOptionsModel())
    {
        ...
        if (model->wallet().hasExternalSigner()) {
            ...
            ui->sendButton->setToolTip(tr("Creates a Partially Signed Quranium Transaction (PSBT) for use with e.g. an offline Quranium Core wallet, or a PSBT-compatible hardware wallet.").arg(PACKAGE_NAME));
        }
...
bool SendCoinsDialog::PrepareSendText(QString& question_string, QString& informative_text, QString& detailed_text)
{
    ...
    if (model->wallet().privateKeysDisabled() && !model->wallet().hasExternalSigner()) {
        ...
        question_string.append(tr("Please, review your transaction proposal. This will produce a Partially Signed Quranium Transaction (PSQT) which you can save or copy and then sign with e.g. an offline Quranium Core wallet, or a PSBT-compatible hardware wallet.").arg(PACKAGE_NAME));
        ...
        question_string.append(tr("Please, review your transaction. You can create and send this transaction or create a Partially Signed Quranium Transaction (PSQT), which you can save or copy and then sign with, e.g., an offline Quranium Core wallet, or a PSBT-compatible hardware wallet.").arg(PACKAGE_NAME));
    } else {
    ...
    if(txFee > 0)
    {
        ...
        question_string.append(" (" + tr("%1 kvB", "PSQT transaction creation").arg((double)m_current_transaction->getTransactionSize() / 1000, 0, 'g', 3) + "): ");
...
void SendCoinsDialog::presentPSBT(PartiallySignedTransaction& psbtx)
{
    ...
    case QMessageBox::Save: {
        ...
        QString filename = GUIUtil::getSaveFileName(this,
            ...
            tr("Partially Signed Transaction (Binary)") + QLatin1String(" (*.psqt)"), &selectedFilter);
        if (filename.isEmpty()) {
            return;
        }
        ...
        Q_EMIT message(tr("PSQT saved"), tr("PSQT saved to disk"), CClientUIInterface::MSG_INFORMATION);
        break;
    }
...
bool SendCoinsDialog::signWithExternalSigner(PartiallySignedTransaction& psbtx, CMutableTransaction& mtx, bool& complete) {
    ...
    if (err) {
        tfm::format(std::cerr, "Failed to sign PSQT");
...
void SendCoinsDialog::coinControlChangeEdited(const QString& text)
{
    if (model && model->getAddressTableModel())
    {
        ...
        {
            ui->labelCoinControlChangeLabel->setText(tr("Warning: Invalid Quranium address"));
        }
        else // Valid address
        {
